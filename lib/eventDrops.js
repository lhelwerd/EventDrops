...
function eventDrops(config = {}) {
    ...
    const xScale = (width, timeBounds) => {
        return finalConfiguration.xScale(d3.time.scale()
            .range([0, width])
            .domain(timeBounds));
    };
    ...
    function eventDropGraph(selection) {
        return selection.each(function selector(data) {
            ...
            const scales = {
                x: xScale(dimensions.width - finalConfiguration.labelsWidth - finalConfiguration.labelsRightMargin, [finalConfiguration.start, finalConfiguration.end]),
                y: yScale(data),
            };
